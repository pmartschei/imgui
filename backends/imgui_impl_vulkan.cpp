...
//  [x] Renderer: User texture binding. Use 'VkDescriptorSet' as ImTextureID. Read the FAQ about ImTextureID! See https://github.com/ocornut/imgui/pull/914 for
//  discussions. [X] Renderer: Large meshes support (64k+ vertices) with 16-bit indices. [x] Renderer: Multi-viewport / platform windows. With issues
//  (flickering when creating a new viewport).
...
//  api symetry. (#914, #5738). 2022-01-20: Vulkan: Added support for ImTextureID as VkDescriptorSet. User need to call ImGui_ImplVulkan_AddTexture(). Building
...
//  initial frame has no vertices. 2020-04-26: Vulkan: Fixed edge case where render callbacks wouldn't be called if the ImDrawData didn't have vertices.
//  2019-08-01: Vulkan: Added support for specifying multisample count. Set ImGui_ImplVulkan_InitInfo::MSAASamples to one of the VkSampleCountFlagBits values to
//  use, default is non-multisampled as before. 2019-05-29: Vulkan: Added support for large mesh (64K+ vertices), enable ImGuiBackendFlags_RendererHasVtxOffset
//  flag. 2019-04-30: Vulkan: Added support for special ImDrawCallback_ResetRenderState callback to reset render state. 2019-04-04: *BREAKING CHANGE*: Vulkan:
//  Added ImageCount/MinImageCount fields in ImGui_ImplVulkan_InitInfo, required for initialization (was previously a hard #define IMGUI_VK_QUEUED_FRAMES 2).
//  Added ImGui_ImplVulkan_SetMinImageCount(). 2019-04-04: Vulkan: Added VkInstance argument to ImGui_ImplVulkanH_CreateWindow() optional helper. 2019-04-04:
//  Vulkan: Avoid passing negative coordinates to vkCmdSetScissor, which debug validation layers do not like. 2019-04-01: Vulkan: Support for 32-bit index
//  buffer (#define ImDrawIdx unsigned int). 2019-02-16: Vulkan: Viewport and clipping rectangles correctly using draw_data->FramebufferScale to allow retina
//  display. 2018-11-30: Misc: Setting up io.BackendRendererName so it can be displayed in the About Window. 2018-08-25: Vulkan: Fixed mishandled
//  VkSurfaceCapabilitiesKHR::maxImageCount=0 case. 2018-06-22: Inverted the parameters to ImGui_ImplVulkan_RenderDrawData() to be consistent with other
//  backends. 2018-06-08: Misc: Extracted imgui_impl_vulkan.cpp/.h away from the old combined GLFW+Vulkan example. 2018-06-08: Vulkan: Use draw_data->DisplayPos
//  and draw_data->DisplaySize to setup projection matrix and clipping rectangle. 2018-03-03: Vulkan: Various refactor, created a couple of
//  ImGui_ImplVulkanH_XXX helper that the example can use and that viewport support will use. 2018-03-01: Vulkan: Renamed ImGui_ImplVulkan_Init_Info to
//  ImGui_ImplVulkan_InitInfo and fields to match more closely Vulkan terminology. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback,
//  ImGui_ImplVulkan_Render() calls ImGui_ImplVulkan_RenderDrawData() itself. 2018-02-06: Misc: Removed call to ImGui::Shutdown() which is not available
//  from 1.60 WIP, user needs to call CreateContext/DestroyContext themselves. 2017-05-15: Vulkan: Fix scissor offset being negative. Fix new Vulkan validation
//  warnings. Set required depth member for buffer image copy. 2016-11-13: Vulkan: Fix validation layer warnings and errors and redeclare gl_PerVertex.
//  2016-10-18: Vulkan: Add location decorators & change to use structs as in/out in glsl, update embedded spv (produced with glslangValidator -x). Null the
//  released resources. 2016-08-27: Vulkan: Fix Vulkan example for use when a depth buffer is active.
...
static void ImGui_ImplVulkan_SetupRenderState(
    ImDrawData* draw_data, VkPipeline pipeline, VkCommandBuffer command_buffer, ImGui_ImplVulkanH_FrameRenderBuffers* rb, int fb_width, int fb_height)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ...
    // Our visible imgui space lies from draw_data->DisplayPps (top left) to draw_data->DisplayPos+data_data->DisplaySize (bottom right). DisplayPos is (0,0)
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback != nullptr) {
                ...
                if (sizeof(ImTextureID) < sizeof(ImU64)) {
                    // We don't support texture switches if ImTextureID hasn't been redefined to be 64-bit. Do a flaky check that other textures haven't been
                    // used.
    ...
    // We perform a call to vkCmdSetScissor() to set back a full viewport which is likely to fix things for 99% users but technically this is not perfect. (See
    // github #4644)
...
bool ImGui_ImplVulkan_UpdateFontsTexture(VkCommandBuffer command_buffer)
{
    ImGuiIO& io = ImGui::GetIO();
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
...
// Register a texture
// FIXME: This is experimental in the sense that we are unsure how to best design/tackle this problem, please post to https://github.com/ocornut/imgui/pull/914
...
VkPresentModeKHR
ImGui_ImplVulkanH_SelectPresentMode(VkPhysicalDevice physical_device, VkSurfaceKHR surface, const VkPresentModeKHR* request_modes, int request_modes_count)
{
    IM_ASSERT(g_FunctionsLoaded && "Need to call ImGui_ImplVulkan_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
    ...
    wd->SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat(
        v->PhysicalDevice, wd->Surface, requestSurfaceImageFormat, (size_t) IM_ARRAYSIZE(requestSurfaceImageFormat), requestSurfaceColorSpace);
    ...
    ImGui_ImplVulkanH_CreateOrResizeWindow(
        v->Instance, v->PhysicalDevice, v->Device, wd, v->QueueFamily, v->Allocator, (int) viewport->Size.x, (int) viewport->Size.y, v->MinImageCount);
    vd->WindowOwned = true;
}
...
static void ImGui_ImplVulkan_SetWindowSize(ImGuiViewport* viewport, ImVec2 size)
{
    ...
    ImGui_ImplVulkanH_CreateOrResizeWindow(
        v->Instance, v->PhysicalDevice, v->Device, &vd->Window, v->QueueFamily, v->Allocator, (int) size.x, (int) size.y, v->MinImageCount);
}
...
#    ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
        if (v->UseDynamicRendering) {
            ...
            vkCmdPipelineBarrier(
                fd->CommandBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0, 0, nullptr, 0, nullptr, 1, &barrier);
...
#    ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
        if (v->UseDynamicRendering) {
            ...
            barrier.subresourceRange.layerCount = 1;
            vkCmdPipelineBarrier(
                fd->CommandBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0, 0, nullptr, 0, nullptr, 1, &barrier);
        } else
#    endif
        {
